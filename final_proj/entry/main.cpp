...
void airports_subprogram_handle_details(Graph *data, string id) {
    ...
    //this is needed so no new duplicate destination is inserted
    unordered_map<string, Edge*> direct_destinations;

    //vector storing unique international destination this is needed so no new duplicate destination is inserted
    unordered_map<string, Edge*> international_destinations;
    
    //remove duplicate destinations which correspond to different airlines.
    for (Edge* x : all_connections) {
        direct_destinations.insert({x->dest->id, x});
        
        //if the destination and source airport
        if (x->dest->location_country != info->location_country) {
    ...
    //if there are international destinations
    if (international_destinations.size() != 0) {
    ...
    //since all commerical airports are regarded as being connected, these airports are essentially all commerical airports in the world. It is found that the size of this does not depend on which airport is the origin, but the number of hops in a graph of course changes.
    vector<pair<int, string>> all_BFS_connections = data->BFS(id);
...
void airports_subprogram(Graph* data) {
    ...
    //sets if the program is finished or not
    ...
    while (!program_finished) {
        ...
        //exit sequence after input is validated
        validate_option(options, input);
        ...
        if (input == "code") {
            ...
            //handle displaying options
            airports_subprogram_handle_details(data, id);
            ...
            //end the program if exit is typed
...
void data_subprogram(Graph* data) {
    ...
    //stores the information as int to a string value id
    vector<pair<unsigned int, string>> num_international_destinations;

    //stores the information as int to a string value id
    vector<pair<unsigned int, string>> num_total_destinations;

    //stores the number of destinations from each airport that is only one connection away but not a direct flight
    vector<pair<unsigned int, string>> num_one_stop_destinations;
    
    //interate through all nodes
    for (auto pair: data->getNodeMap()) {
        string current_id = pair.first;

        //only run on nodes that have neighbors
        if (!data->getEdgeNeighbors(current_id).empty()) {
            auto all_connections = data->getEdgeNeighbors(current_id);

            //stores current country
            string current_country = pair.second->location_country;

            //this is needed so no new duplicate destination is inserted
            unordered_set<string> international_destinations;

            //this is needed so no new duplicate destination is inserted
            unordered_set<string> total_destinations;

            //remove duplicate destinations which correspond to different airlines.
            for (Edge* e : all_connections) {
                Node* dest = e->dest;

                //if the the destinations country does not match the current country, add it
                if (dest->location_country != current_country) {
                    //maps by city
                    international_destinations.insert(dest->id);
                }

                //add all
                total_destinations.insert(dest->id);
            } 
            
            //run BFS on current airport find the number of airports connected to this airport in some way and to find the number of connections available to this airport and the number of airports that are connected by only one connection or two flights. 
            
            //since all commerical airports are regarded as being connected, these airports are essentially all commerical airports in the world. It is found that the size of this does not depend on which airport is the origin, but the number of hops in a graph of course changes.
            vector<std::pair<int, string>> all_BFS_2_connections = data->BFS(current_id, 2);
            num_one_stop_destinations.push_back({all_BFS_2_connections.size(), current_id});

            //append
            num_international_destinations.push_back({international_destinations.size(), current_id});
            num_total_destinations.push_back({total_destinations.size(), current_id});
        }
    }

    //sort this output in descending order
    sort(num_international_destinations.begin(), num_international_destinations.end(), greater<pair<int, string>>());

    //sort this output in descending order
    sort(num_total_destinations.begin(), num_total_destinations.end(), greater<pair<int, string>>());
    sort(num_one_stop_destinations.begin(), num_one_stop_destinations.end(), greater<pair<int, string>>());

    //initial options
    vector<string> options = {"1", "2", "3", "4", "5", "exit"};

    //stores pagerank info
...
int main() {   
    ...
    //making new graph object
    Graph* data = new Graph(airlines_path, airports_path, flights_path);
    ...
    while (!program_finished) {
        ...
        //exit sequence after input is validated
        validate_option(main_options, input);
        ...
        if (input == "airports") {
            ...
            delete data;
            //end the program if exit is pushed
            program_finished = true;
        }
    }
